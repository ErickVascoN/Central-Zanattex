...
def navegacao(request):
    """Injeta os módulos (agrupados por aba, já filtrados pelo setor do
    usuário — ver contas/permissions.py::modulos_liberados), a aba ativa e
    um índice achatado (nav_search) em todos os templates — o menu lateral
    usa nav_abas pra abrir só o setor da página atual, a busca global
    (Ctrl+K) usa nav_search pra filtrar todos os módulos de uma vez, sem
    duplicar a lista em Python/JS."""
    ...
    return {"nav_abas": modulos_por_aba(modulos_visiveis), "aba_ativa": aba_ativa, "nav_search": nav_search}
